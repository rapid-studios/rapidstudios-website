...
function getCalendlyPopupUrl(theme: SiteTheme) {
  const colors = CALENDLY_THEME_PARAMS[theme];
  const params = new URLSearchParams({
    background_color: colors.backgroundColor,
    hide_landing_page_details: "1",
    primary_color: "ff5449",
...
function getCalendlyInlineUrl(theme: SiteTheme) {
  return `${getCalendlyPopupUrl(theme)}&hide_gdpr_banner=1`;
}

function useCalendlyTheme() {
  const [theme, setTheme] = useState<SiteTheme>(() => (typeof document === "undefined" ? DEFAULT_THEME : readTheme()));
      ...
      {geometry ? (
        <motion.div
          animate={{
            boxShadow:
              panelVisible
                ? "0 34px 96px rgba(4, 10, 18, 0.52), 0 0 0 1px rgba(255, 84, 73, 0.2)"
                : "0 28px 70px rgba(77, 150, 247, 0.18), 0 0 0 1px rgba(77, 150, 247, 0.12)",
        ...
        >
          <motion.button
            ...
            aria-label="Close scheduling panel"
            className="absolute right-4 top-4 z-20 flex size-10 items-center justify-center border border-[var(--color-line-subtle)] bg-[var(--color-surface)] text-[var(--color-text-secondary)] backdrop-blur-sm transition-colors hover:border-[var(--color-brand-primary-strong)] hover:text-[var(--color-text-primary)]"
            initial={false}
            onClick={handleClose}
            transition={{
          ...
          <motion.div
            animate={{ opacity: expanded ? 1 : 0, y: expanded || prefersReducedMotion ? 0 : -6 }}
            className="pointer-events-none absolute left-4 top-4 z-20 flex items-center gap-3 border border-[var(--color-line-subtle)] bg-[var(--color-surface-soft)] px-3 py-2 text-[var(--color-text-secondary)] backdrop-blur-sm"
            initial={false}
            transition={{
              duration: prefersReducedMotion ? 0.08 : 0.12,
              ease: [0.22, 1, 0.36, 1]
          ...
          >
            <BrandIcon className="size-4 text-[var(--color-brand-primary-strong)]" size={16} title={undefined} />
            <span className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-[-0.06em] text-[var(--color-text-primary)]">
              <RapidWordmark className="scale-[0.88]" textClassName="text-[var(--color-brand-primary-strong)]" />
              <span>Studios</span>
            </span>
          </motion.div>
          <motion.div
    ...
    <>
      ...
      >
        <motion.div
          animate={{
            boxShadow: expanded
              ? "0 34px 96px rgba(4, 10, 18, 0.52), 0 0 0 1px rgba(255, 84, 73, 0.18)"
              : "0 28px 70px rgba(77, 150, 247, 0.22), 0 0 0 1px rgba(77, 150, 247, 0.14)",
            clipPath: expanded
              ? "inset(0% 0% 0% 0% round 32px)"
              : `inset(calc(100% - ${collapsedHeight}px) 0% 0% calc(100% - ${collapsedWidth}px) round 999px)`,
            opacity: 1
          }}
          className="pointer-events-none absolute inset-0 overflow-hidden border border-[var(--color-line-subtle)] bg-[var(--color-surface-soft)] backdrop-blur-[20px]"
          initial={{
        ...
        >
          <motion.button
            ...
            type="button"
            whileHover={prefersReducedMotion || expanded ? undefined : { y: -1, scale: 1.004 }}
            whileTap={prefersReducedMotion || expanded ? undefined : { y: 0.5, scale: 0.992 }}
            >
              <motion.span
                aria-hidden="true"
            ...
            >
              ...
              <div className="pointer-events-none absolute left-4 top-4 z-20 flex items-center gap-3 border border-[var(--color-line-subtle)] bg-[var(--color-surface-soft)] px-3 py-2 text-[var(--color-text-secondary)] backdrop-blur-sm">
                <BrandIcon className="size-4 text-[var(--color-brand-primary-strong)]" size={16} title={undefined} />
                <span className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-[-0.06em] text-[var(--color-text-primary)]">
                  <RapidWordmark className="scale-[0.88]" textClassName="text-[var(--color-brand-primary-strong)]" />
                  <span>Studios</span>
